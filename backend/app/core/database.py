...
# Build RDS URL (FIXED - was missing before)
RDS_URL = None
if settings.RDS_HOST and settings.RDS_HOST != "localhost":
    RDS_URL = (
        f"postgresql+psycopg2://{settings.RDS_USER}:"
        f"{settings.RDS_PASSWORD}@{settings.RDS_HOST}:"
        f"{settings.RDS_PORT}/{settings.RDS_DB}"
    )
...
if settings.RDS_HOST and settings.RDS_HOST != "localhost" and "sqlite" not in str(DATABASE_URL):
    ...
    if RDS_URL and test_connection(RDS_URL):
